...
import uvicorn
from pymessenger import Bot, bot
...
async def english_bot(sender , bot_message):

    r=requests.post('http://localhost:5005/webhooks/rest/webhook' , json={"sender": sender, "message":bot_message})
    ...
    for i in list_of_dict:
        ...
        await send_message(page_access_token= PAGE_ACCESS_TOKEN , recipient_id = sender, message_text = bot_message)


async def hindi_bot(sender , bot_message):

    r=requests.post('http://localhost:5002/webhooks/rest/webhook' , json={"sender": sender, "message":bot_message})
            ...
            for messaging_event in entry['messaging']:
                ...
                if messaging_event.get('message'):
                    ...
                    if 'nlp' in messaging_event['message']:
                        nlp_message = messaging_event['message']['nlp']
                        for detected_locales in nlp_message['detected_locales']:
                            language = detected_locales['locale']
                     
                    if language == "en_XX" or language == "hi_IN" or language == "en":
                        await english_bot(sender_id,messaging_text)
                     
                    else:
                        await hindi_bot(sender_id,messaging_text)
